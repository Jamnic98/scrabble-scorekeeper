...
function BoardWidget({ players, setPlayers, getCurrentPlayer, turnCount, setTurnCount, setTurnScore }) {

  const [letters, setLetters] = useState([])
  const [skipCount, setSkipCount] = useState(0)
  const [mainWord, setMainWord] = useState([])
  const [previousMainWords, setPreviousMainWords] = useState([])
  const [words, setWords] = useState([])
  const [activeSquareCoords, setActiveSquareCoords] = useState([])
  const [wordDirection, setWordDirection] = useState("")
  const [boardState, setBoardState] = useState([])
  const [previousBoardStates, setPreviousBoardStates] = useState([/* 
 ...
 */
  ...
  const [lastBoardState, setLastBoardState] = useState([[
    // ROW 1
    [{ letter: "", scoreMultiplier: "tw", isBlank: false, isFocused: false },
    { letter: "", scoreMultiplier: "", isBlank: false, isFocused: false },
    { letter: "", scoreMultiplier: "", isBlank: false, isFocused: false },
  ...
  const makeLetterMove = (letter, isBlank = false) => {
    const [x, y] = activeSquareCoords
    const letterObject = { letter: letter, scoreMultiplier: boardState[y][x].scoreMultiplier, isBlank: isBlank }
  ...
  const appendHorizontalSquares = (letterObject) => {
    const lettersToRight = getSquaresToRight(...activeSquareCoords)
    const squaresToRight = lettersToRight.map((letter) => {
      return { letter: letter.letter, scoreMultiplier: "", isBlank: letter.isBlank };
    })

    const lettersToLeft = getSquaresToLeft(...activeSquareCoords).reverse()
  ...
  const appendVerticalSquares = (letterObject) => {
    const lettersBelow = getSquaresBelow(...activeSquareCoords)
    const squaresBelow = lettersBelow.map((letter) => {
      return { letter: letter.letter, scoreMultiplier: "", isBlank: letter.isBlank };
    })

    const lettersAbove = getSquaresAbove(...activeSquareCoords).reverse()
  ...
  const shiftFocus = () => {
    if (wordDirection !== "") {
      const newCoords = getNextAvailableSquare(...activeSquareCoords)
      setActiveSquareCoords(newCoords)
    }
  }

  const getNextAvailableSquare = (x, y) => {
    switch (wordDirection) {
      case "right":
        if (x < 14) {
          const nextX = x + 1
  ...
  const invalidWordsAlert = (invalidWords) => {
    const message = invalidWords.slice(0, invalidWords.length - 1).map((word, index) => {
      if (index <= invalidWords.length - 1) {
        return ` '${word}'`;
      }
    })
    alert(`${message} ${(invalidWords.length > 1) ? "and" : ""} '${invalidWords[invalidWords.length - 1]}' ${(invalidWords.length === 1) ? "is" : "are"} not in the english dictionary!`)
  }
  ...
  const handleBackSpaceKey = () => {
    if (letters.length > 0) {
      ...
      if (previousLetter.isBlank) {
        ...
        remainingLetters[`${previousLetter.letter}`] = remainingLetters[`${previousLetter.letter}`] + 1
      }

      const focusedSquareCoords = getFocusedSquareCoords(previousBoardStates[previousBoardStates.length - 1])


      switch (wordDirection) {
        case "down":
          if (getSquaresToLeft(...focusedSquareCoords).length !== 0 || getSquaresToRight(...focusedSquareCoords).length !== 0) {
            setWords([...words.slice(0, words.length - 1)])
          }
          setActiveSquareCoords(focusedSquareCoords)
          break;
        case "right":
          if (getSquaresAbove(...focusedSquareCoords).length !== 0 || getSquaresBelow(...focusedSquareCoords).length !== 0) {
            setWords([...words.slice(0, words.length - 1)])
          }
          setActiveSquareCoords(focusedSquareCoords)
          break;
  ...
  const handleKeyPressed = (e) => {
    ...
    if (activeSquareCoords.length !== 0) {
      if (isSquareEmpty(...activeSquareCoords) && letters.length < 7) {
