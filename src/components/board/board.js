...
function Board({
  boardState, setBoardState, activeSquareCoords,
  setActiveSquareCoords, wordDirection, setWordDirection, letters }) {
  ...
  const createRowOfSquares = (rowIndex) => {
    return getRowScoreMultipliers(rowIndex).map(_scoreMultiplier => {
      return { letter: "", scoreMultiplier: _scoreMultiplier, isBlank: false, isFocused: false }
    });
  }
